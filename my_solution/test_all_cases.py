"""
Test script for all 4 test cases
"""
...
def test_single_case(system: L2DiagnosticSystem, case_name: str, alert_text: str):
    ...
    with open(output_file, 'w') as f:
        f.write(f"# {case_name.upper().replace('_', ' ')}\n\n")
        f.write(f"## Original Alert\n\n```\n{alert_text}\n```\n\n")
        f.write(f"## Diagnostic Report\n\n{result['report']}\n\n")
        f.write(f"## Technical Details\n\n")
        f.write(f"### Parsed Information\n```json\n{json.dumps(result['parsed'], indent=2)}\n```\n\n")
        f.write(f"### Root Cause Analysis\n```json\n{json.dumps(result['root_cause'], indent=2)}\n```\n\n")
        f.write(f"### Resolution\n```json\n{json.dumps(result['resolution'], indent=2)}\n```\n\n")
